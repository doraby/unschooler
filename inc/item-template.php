...
//    в php это будет выглядеть 'Hello'.' World!!!'). Поэтому в php можно получит данные из именновоного массива только через
//    квадратные скобки.
    ?>
    <div class="item__title">
        <?php echo $item['title']; // Для вывода текстовых данных используется оператор echo?>
    </div>
    <?php if ($item['description']): ?>
        <?= $item['description']; // Короткая запись оператора echo (на некоторых серверах короткие записи оператров могут быть запрещены в настрокакх php) ?>
    <?php endif; ?>
    <?php if ($item['img']): ?>
